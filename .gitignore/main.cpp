...
map<int,int> map_dummy;
map<int,int>::iterator it_map_dummy;
...
if(entry->type == BGPDUMP_TYPE_ZEBRA_BGP && entry->subtype == BGPDUMP_SUBTYPE_ZEBRA_BGP_STATE_CHANGE && entry->length == 8) return;
	...
	case BGPDUMP_TYPE_ZEBRA_BGP:
		...
		dest_as=entry->body.zebra_state_change.destination_as;
...
void calculate_data()
{
	vector<map<int,int>> all_maps;
	vector<map<int,int>>::iterator it_all_maps;	
	...
	broadcast(world,hops_vector, 0);
	broadcast(world,map_degree, 0);
	broadcast(world,map_c2p,0);
	broadcast(world,map_p2c,0);
	...
	for(int i=world.rank()*counter/world.size();i<counter; ++i)
	...
	gather(world, map_c2p,all_maps,0);
	for(int process=0;process < world.size(); ++process)
	{
	map_dummy=all_maps[process];
	map_c2p.insert(map_dummy.begin(),map_dummy.end());
	}
	gather(world, map_p2c,all_maps,0);
	for(int process=0;process < world.size(); ++process)
	{
	map_dummy=all_maps[process];
	map_p2c.insert(map_dummy.begin(),map_dummy.end());
	}
	broadcast(world,map_c2p,0);
	broadcast(world,map_p2c,0);
	broadcast(world,map_p2p,0);
	...
	for(int m=world.rank()*counter/world.size();m<counter;m++)
	{
		vector<string> sstrings;
		sstrings.clear();
		hop_degree_max=0;
	...
	gather(world, map_p2p,all_maps,0);
	for(int process=0;process < world.size(); ++process)
	{
		map_dummy=all_maps[process];
		map_p2p.insert(map_dummy.begin(),map_dummy.end());
	}
	broadcast(world,map_p2p,0);
	...
	for(int p=world.rank()*counter/world.size();p<counter;p++)
	{
		vector<string> sstrings;
		sstrings.clear();
